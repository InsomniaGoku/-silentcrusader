...
class Tick:
    ...
    def __init__(self):
        self.input_handlers = {}
        self.input_handlers["csv"]  = self.read_from_csv
        self.input_handlers["wind"] = self.read_from_wind
        self.output_handler = {}
        self.output_handler["default"] = print # default output, just print these shit
    ...
    def load_input_handler(self,target_format,new_handler):
        if target_format in self.input_handlers.keys():
            print ("overwrite current input handler ",target_format," with new one.\n")
        else:
            print ("add new input handler for",target_format,".\n")
        self.input_handlers[target_format] = new_handler

    def load_output_handler(self,target_format,new_handler):
        if target_format in self.output_handler.keys():
            print ("overwrite current output handler ",target_format," with new one.\n")
        else:
            print ("add new output handler for",target_format,".\n")
    ...
    def play(self,output_format="default"):
        ...
        self.bind_output_method(output_format)
